...
public class MemberController {
	...
	// 비밀번호 암호화를 위해 사용하기 위한 passwordEncoder 전역변수 선언
	@Autowired
	...
	public Map<String, Boolean> login(@RequestBody MemberDto dto, HttpSession session) {
		...
		// 만일 로그인에 성공했다면
		if(res != null) {
			// dto.getMemberpw() : 사용자가 입력한 pw
			// res.getMemberpw() : db에 암호화되어 저장된 pw
			// passwordEncoder은 내부적으로 사용자가 입력한 pw와 db에 암호화되어 저장된 것을 매칭해준다.
	...
	// 만일 Regist 버튼을 클릭하게 되면 넘어오는 값 registform.do를 HanlderMapping을 통해서
	// 아래의 method와 연결해준다.
	...
	// id,pw,name을 다 입력하고 ok버튼을 누르게 되면 HandlerMapping을 통해서
	// 현재 method와 연결해준다.
	@RequestMapping(value="registres.do", method=RequestMethod.POST)
	public String registRes(Model model, MemberDto dto) {
		
		// 회원가입 할 때 사용자가 입력한 비밀번호를 암호화 해서 출력해줄 것이다.
		// 이미 만들어져 있는 아이디는 불가능하며, 새로운 회원으로 가입했을때만 가능하다.
		dto.setMemberpw(passwordEncoder.encode(dto.getMemberpw())); // 사용자가 입력한 비밀번호 : dto.getMemberpw()를 passwordEncoder.encode를 통해 암호화
		logger.info("\n 회원가입 시 암호화 된 memberpw : " + dto.getMemberpw());
